...
python3 ./aligned_icore_exp8mers_single_mc.py -icsdir \${ICSDIR} -trainset \${TRAINSET} -ncores 1 -outdir \${OUTDIR} -condition \${CONDITION} -mc_index \${INDEX}
...
qsub -W group_list=vaccine -A vaccine -m e -l nodes=1:ppn=1:thinnode,mem=5gb,walltime=$WALLTIME "./230414_aligned_expr_mut_oh_filt_single_${1}_exp8mers_index_${2}.sh"
