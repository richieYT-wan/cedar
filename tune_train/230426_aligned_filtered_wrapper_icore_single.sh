...
if [[ $INDEX =~ ^[0-9]+$ ]]; then
  # Create the script for numeric INDEX
  echo "#! /usr/bin/bash
  ...
  # Create the script for non-numeric INDEX
  echo "#! /usr/bin/bash
  ...
  pwd
  echo \"Starting PyScript\"
  python3 ./230427_final_mutExpr_single_mc.py -icsdir \${ICSDIR} -trainset \${TRAINSET} -ncores 1 -outdir \${OUTDIR} -condition \${CONDITION} -key \${INDEX} -input_type \${INPUTTYPE}
  " > "230427_MCFinal_${1}_exp8mers_index_${2}_input_type_${3}.sh"
fi
