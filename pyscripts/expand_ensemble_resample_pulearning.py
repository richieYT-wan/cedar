...
def final_bootstrap_wrapper(preds_df, args, filename, expand, subsample, condition, evalset,
    ...
    bootstrapped_df['subsample'] = subsample
...
def parallel_inner_train_wrapper(train_dataframe, x_test, base_model, ics_dict,
                                 encoding_kwargs, standardize, fold_out, fold_in, n_resample=2, subsample=1.0):
    ...
    # Here re-sample 10x
    for resample in range(1,n_resample+1):
...
def nested_kcv_train_sklearn_expand(dataframe, base_model, ics_dict, encoding_kwargs: dict = None, n_jobs: int = None, n_resample=2, subsample=1.0):
    """
    Args:
        dataframe:
        base_model:
    ...
    for fold_out in tqdm(folds, leave=False, desc='Outer fold', position=2):
        ...
        train_wrapper_ = partial(parallel_inner_train_wrapper, train_dataframe=dataframe, x_test=x_test,
                                 ...
                                 standardize=std, fold_out=fold_out, n_resample=n_resample, subsample=subsample)
        ...
        for expand_ensemble in reversed([1, 2, 5, 10]):
            for subsample in reversed([0.5, 0.75, 1.0]):
                ...
                for evalset, evalname in zip([cedar_dataset, prime_dataset, nepdb_dataset],
                    ...
                    bootstrapped_df = final_bootstrap_wrapper(preds, args, filename, expand_ensemble, subsample, condition, 
