...
def args_parser():
    ...
    parser.add_argument('-ncores', type=int, default=8, help='N cores to use in parallel, default = 8')
    parser.add_argument('-gpu', type=str2bool, default=False, help='Enable GPU')
    # parser.add_argument('-outname', type=str, default='_xgb_hp', help='additional name (appended to the end)')
...
def main():
    ...
    if args['debug']:
        models_params_grid = zip([RandomForestClassifier(), XGBClassifier(n_jobs=1,
    ...
    for model, hyperparameters in tqdm(models_params_grid, desc='models', leave=False):
        ...
        output = Parallel(n_jobs=args['ncores'])(delayed(wrapper)(train_dataset=train_dataset, encoding=encoding,
                                                                  blosum_matrix=blosum_matrix, ics_dict=ics_dict,
                                                                  ics_name=ics_name,
                                                                  mask=mask, add_rank=add_rank, add_aaprop=add_aaprop,
                                                                  remove_pep=remove_pep, standardize=standardize) for \
                                                 (train_dataset, encoding, blosum_matrix, ics_dict, ics_name, mask,
                                                  add_rank,
