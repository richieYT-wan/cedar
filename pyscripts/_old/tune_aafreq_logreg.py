...
def args_parser():
    ...
    parser.add_argument('-outname', type=str, default='logreg', help='additional name (appended to the end)')
...
def main():
    ...
    if args['debug']:
        ...
        tune_results_model = {}
    print(f'\n\n\n\tCPU COUNT: {multiprocessing.cpu_count()}, using {multiprocessing.cpu_count()-2}\n\n\n')
    ...
    for model, hyperparameters in tqdm(models_params_grid, desc='models', leave=False):
        ...
        output = Parallel(n_jobs=multiprocessing.cpu_count())(delayed(wrapper)(train_dataset=train_dataset, encoding=encoding,
