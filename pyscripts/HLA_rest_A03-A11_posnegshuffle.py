...
def args_parser():
    ...
    parser.add_argument('-outdir', type=str, default='../output/230119_rest_AXX_posnegshuffle/')
    ...
    parser.add_argument('-input_type', type=str, default='icore_mut', help = 'Peptide or icore_mut ; Will change the rank col')
...
def main():
    ...
    rank_map = {'icore_mut':'EL_rank_mut',
                'Peptide':'trueHLA_EL_rank'}
    assert args['input_type'] in rank_map.keys(), f'Wrong input type. Expected one of {rank_map.keys()}, got {args["input_type"]}'
    ...
    encoding_kwargs = dict(max_len=12, encoding='onehot', blosum_matrix=None, mask=False, add_rank=True,
                           ...
                           target_col='agg_label', seq_col=args['input_type'], rank_col=rank_map[args['input_type']], hla_col='HLA',
                           mask_aa=False, mut_col = [])
    ...
    top_hlas = ['HLA-A0201', 'HLA-A2402', 'HLA-B0207', 'HLA-B1501', 'HLA-B3501']
    hlas_remove = ['HLA-A0301', 'HLA-A1101']

    cedar_tops = cedar_dataset.query('HLA in @top_hlas').reset_index(drop=True).copy()
    cedar_rest_a03a11 = cedar_dataset.query('HLA not in @top_hlas').reset_index(drop=True).copy()
    cedar_a03a11 = cedar_rest_a03a11.query('HLA in @hlas_remove').reset_index(drop=True).copy()

    # Baseline is cedar_rest
    cedar_rest_no_axx_negs = cedar_rest_a03a11.drop(index=cedar_rest_a03a11.query('HLA in @hlas_remove and agg_label==0').index)
    cedar_rest_no_axx_pos = cedar_rest_a03a11.drop(index=cedar_rest_a03a11.query('HLA in @hlas_remove and agg_label==1').index)
    cedar_rest_no_axx = cedar_rest_a03a11.query('HLA not in @hlas_remove')

    for train_dataset, trainname in zip([cedar_rest_a03a11, cedar_rest_no_axx_negs, cedar_rest_no_axx_pos, cedar_rest_no_axx, cedar_tops],
                                        ['rest+AXX', 'rest_noAXXneg', 'rest_noAXXpos', 'rest_noAXX', 'Top HLAs']):
        ...
        for invert in [True, False]:
            for ic_name, ics_dict in tqdm(
                ...
                for evalset, evalname in zip(
                        [cedar_a11_rest, cedar_rest_no_axx_pos, cedar_rest_no_axx_negs, cedar_rest_no_axx,
                         cedar_tops, prime_dataset, ibel_dataset],
                        ['rest+AXX', 'rest_noAXXneg', 'rest_noAXXpos', 'rest_noAXX', 'top HLAs', 'PRIME','IBEL']):
