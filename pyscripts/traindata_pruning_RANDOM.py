...
def parallel_inner_train_wrapper_prune(train_dataframe, x_test, base_model, ics_dict,
                                       encoding_kwargs, standardize, fold_out, fold_in, percentile, bot, top, model_name):
    ...
    # TODO: Here get the numbers / stats and randomly remove X positive and Y negative datapoints
    train = train_dataframe.query('fold != @fold_out and fold != @fold_in').reset_index(drop=True)
    # Rereading the preds and getting the number of mispredicted points to remove from a given fold
    df = pd.read_csv(
        f'/home/projects/vaccine/people/yatwan/cedar/output/230724_TrainDataPruning_CompareModels/raw/KCV_preds_{model_name}_PruningTrue_Percentile_{percentile:02}.csv').query(
        'fold!=@fold_in and fold !=@fold_out')
    ...
    # dropping index
    train = train.drop(pos_index.join(neg_index, how='outer'))
...
def nested_kcv_train_sklearn_prune(dataframe, base_model, ics_dict, encoding_kwargs: dict = None, n_jobs: int = None,
                                   percentile=1, preds_100k = None, model_name='consensus'):
    """
    Args:
        dataframe:
        base_model:
    ...
    bot, top = [percentile / 100, (100 - percentile) / 100]
    ...
    for fold_out in tqdm(folds, leave=False, desc='Train:Outer fold', position=2):
        ...
        train_wrapper_ = partial(parallel_inner_train_wrapper_prune, train_dataframe=dataframe, x_test=x_test,
                                 ...
                                 standardize=std, fold_out=fold_out, percentile=percentile, bot=bot, top=top, model_name=model_name)
...
def args_parser():
    ...
    parser.add_argument('-outdir', type=str, default='../output/230804_RandomPruning/')
    ...
    for kwargs, ics, name in [base, consensus]:
        ...
        for percentile_thr in range(1, 16):
            ...
            if os.path.exists(f'{args["outdir"]}raw/featimps_{name}_PrunedTrue_Percentile_{percentile_thr:02}.csv'):
                continue
            print(f'Training randomly pruned {name} model')
            ...
            _, prime_preds_prune = evaluate_trained_models_sklearn(
            ...
            _, nepdb_preds_prune = evaluate_trained_models_sklearn(
                nepdb_dataset.query('Peptide not in @cedar_dataset.Peptide.values'),
                trained_models_prune, ics, None,
                kwargs, concatenated=False, only_concat=False, n_jobs=10,
                kcv_eval=False)

            for preds, evalname, pruned in [
