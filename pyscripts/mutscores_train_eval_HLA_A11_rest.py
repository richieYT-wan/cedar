...
def main():
    ...
    for train_dataset, trainname in zip([cedar_a11_rest, cedar_tops], ['cedar_a11_rest', 'cedar_tops']):
        # Manually gets split everytime
        # Do weights loops
        hla = trainname.split('cedar_')[1]
        for mut_cols in tqdm(mcs, position=0, leave=True, desc='cols'):
            ...
            for invert in [True, False]:
                for ic_name, ics_dict in tqdm(
                    ...
                    # creating filename
                    filename = f'{trainname}_onehot_{"-".join(ic_name.split(" "))}_{hla}_{key}'
