...
def main():
    ...
    if args["debug"]:
        ...
        train_datasets = [dataset_prime, dataset_viral, dataset_cedar_viral]
        standardize_ = [True]
    ...
    if args['debug']:
        ...
        models_params_grid = zip([XGBClassifier(method='hist', n_jobs=1)],
    ...
    for model, hyperparameters in tqdm(models_params_grid, desc='models', leave=False):
        ...
        output = Parallel(n_jobs=multiprocessing.cpu_count()-4)(delayed(wrapper)(train_dataset=train_dataset, encoding=encoding,
